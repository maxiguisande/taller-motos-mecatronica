...
/** Admin siempre; el mecánico asignado solo si la orden NO está finalizada. */
function puede(
  user: User,
  ...
  return !!user.id && mecanicoId === user.id && !finalizadoEn;
}
...
export async function POST(req: Request) {
  ...
  if (ordenItemId) {
    ...
    const item = await prisma.ordenItem.findUnique({
      ...
      include: {
        orden: { select: { id: true, mecanicoId: true, finalizadoEn: true } },
        _count: { select: { fotos: true } },
      },
    });
    if (!item) return NextResponse.json({ error: "Ítem inexistente" }, { status: 404 });
    if (!puede(user, item.orden.mecanicoId, item.orden.finalizadoEn))
    ...
    const orden = await prisma.ordenTrabajo.findUnique({
      ...
      select: {
        ...
        mecanicoId: true,
        finalizadoEn: true,
        _count: { select: { fotos: { where: { categoria } } } },
      },
    });
    if (!orden) return NextResponse.json({ error: "Orden inexistente" }, { status: 404 });
    if (!puede(user, orden.mecanicoId, orden.finalizadoEn))
      return NextResponse.json({ error: "Sin permiso" }, { status: 403 });
...
export async function DELETE(req: Request) {
  ...
  const foto = await prisma.foto.findUnique({
    where: { id },
    include: {
      item: { include: { orden: { select: { id: true, mecanicoId: true, finalizadoEn: true } } } },
      orden: { select: { id: true, mecanicoId: true, finalizadoEn: true } },
    },
  });
  ...
  if (!orden || !puede(user, orden.mecanicoId, orden.finalizadoEn))
    return NextResponse.json({ error: "Sin permiso" }, { status: 403 });
